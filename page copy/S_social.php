...
<?php
    ...
    <div class="px-4 py-5 my-5 text-center">
        ...
        <h1 class="display-5 fw-bold text-body-emphasis">Social Site</h1>
        <div class="col-lg-6 mx-auto">
            <p class="lead mb-4">สังคมสุดสนุก</p>
            <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
                <button type="button" class="btn btn-outline-secondary btn-lg px-4" ><a href="#SO1">ประถม</a></button>
                <button type="button" class="btn btn-outline-secondary btn-lg px-4" ><a href="#SO2">ม.ต้น</a></button>
                <button type="button" class="btn btn-outline-secondary btn-lg px-4" ><a href="#SO3">ม.ปลาย</a></button>
            </div>
        </div>
...
<div class="container marketing">
    ...
    <div class="row ">
        <!-- วิชาสังคม  -->
        <?php  
            $stmt = $conn->prepare("SELECT * FROM Courses WHERE category LIKE 'SO1' ORDER BY updated_at DESC ");
            $stmt->execute();
        ...
        <div class="album py-5 bg-body-tertiary">
            <h2 class="featurette-heading fw-normal lh-1" id="SO1">ระดับประถม <span class="text-body-secondary"> .... </span></h2>
            <br>
            <div class="container">
        ...
        <hr class="#">
         <!-- วิชาสังคม มัธยมต้น -->
         <?php  
            $stmt = $conn->prepare("SELECT * FROM Courses WHERE category LIKE 'SO2' ORDER BY updated_at DESC ");
            $stmt->execute();
            $rowCount = $stmt->rowCount();
        ?>
        <?php if($rowCount > 0 ): ?>
        ...
        <div class="album py-5 bg-body-tertiary">
            <h2 class="featurette-heading fw-normal lh-1" id="SO2">ระดับมัธยมต้น<span class="text-body-secondary"> .... </span></h2>
            <br>
            <div class="container">
        ...
        <hr class="#">
         <!-- วิชาสังคม มัธยมปลาย -->
         <?php  
            $stmt = $conn->prepare("SELECT * FROM Courses WHERE category LIKE 'SO3' ORDER BY updated_at DESC ");
            $stmt->execute();
            $rowCount = $stmt->rowCount();
        ?>
        <?php if($rowCount > 0 ): ?>
        ...
        <div class="album py-5 bg-body-tertiary">
            <h2 class="featurette-heading fw-normal lh-1" id="SO3">ระดับมัธยมปลาย<span class="text-body-secondary"> .... </span></h2>
            <br>
            <div class="container">
